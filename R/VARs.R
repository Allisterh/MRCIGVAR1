...
#' @param r_np  : n x p matrix of roots outside the unit circle for an n dimensional independent ar(p)-processes (the roots of the characteristic functions in the lag operator.). If not provided, it will be generated randomly.
#' @param A     : an n x n full rank matrix of transformation to generate correlated VAR(p) from the n independent AR(p)  
#' @param B     : (n,n,p) array of the VAR(p) process. If B is not given, it will be calculated out of r_np and A.
#' @param Co    : (n,k+1) vector of intercept of the VAR(p) process. for type="none" Co = 0*(1:n), for const Co is an n vector, exog0 Co is a (n,k) matrix for exog1 Co is an (n,1+k) matrix. 
#' @param U     : residuals, if it is not NA it will be used as input to generate the VAR(p)
#' @param Sigma : The covariance matrix of the VAR(p)
...
#' @return      : An object of VAR(p) containing the generated data, the parameters and the input exogeous variables. res = list(n,p,type,r_np,Phi,A,B,Co,Sigma,Y,X,resid,U,Y1,Yo,check)
#' @examples 
...
if ( n>1 ) {
	if (res$type=="none")   varp = vars::VAR(y=Y, p = p, type = "none")
	if (res$type=="const")  varp = vars::VAR(y=Y, p = p, type = "const")
	if (res$type=="exog0")  varp = vars::VAR(y=Y, p = p, type = "none",exogen=X)
	if (res$type=="exog1")  varp = vars::VAR(y=Y, p = p, type = "const",exogen=X)
}  else  {
   varp = LREG
}
res$varp = varp
